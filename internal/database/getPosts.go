...
func GetAllPosts() ([]models.Post, error) {
	...
	rows, err := DB.Query("SELECT id, user_id, content, image_path, created_at,title FROM posts")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetPostsByCategoryID(categoryID int) ([]models.Post, error) {
	...
	for rows.Next() {
		var post models.Post
		var imagePath sql.NullString
		if err := rows.Scan(&post.ID, &post.AuthorID, &post.Title, &post.Content, &imagePath, &post.CreatedAt); err != nil {
			return nil, err
		}
		if imagePath.Valid {
			post.ImagePath = imagePath.String
